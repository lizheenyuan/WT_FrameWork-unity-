...
public class MediaPlayerCtrl : MonoBehaviour
{
    ...
    [DllImport("EasyMovieTexture")]
    private static extern void SetTextureFromUnity(int iID, System.IntPtr texture, System.IntPtr textureY,
        System.IntPtr textureU, System.IntPtr textureV, int w, int h, byte[] data);
    ...
    static MediaPlayerCtrl()
    {
        ...
        dllPath = Environment.CurrentDirectory + Path.DirectorySeparatorChar + "Assets" + Path.DirectorySeparatorChar +
                  "Plugins" + Path.DirectorySeparatorChar + "x64";
        ...
        dllPath = Environment.CurrentDirectory + Path.DirectorySeparatorChar + "Assets" + Path.DirectorySeparatorChar +
                  "Plugins" + Path.DirectorySeparatorChar + "x86";
	...
	Call_Load(strName,0);
        ...
        if (m_CurrentState == MEDIAPLAYER_STATE.PLAYING || m_CurrentState == MEDIAPLAYER_STATE.PAUSED)
        {
            if (m_bCheckFBO == false)
            {
                ...
                {
                    m_VideoTextureY = new Texture2D(Call_GetVideoWidth(), Call_GetVideoHeight(), TextureFormat.Alpha8,
                        false);
                    m_VideoTextureU = new Texture2D(Call_GetVideoWidth()/2, Call_GetVideoHeight()/2,
                        TextureFormat.Alpha8, false);
                    m_VideoTextureV = new Texture2D(Call_GetVideoWidth()/2, Call_GetVideoHeight()/2,
                        TextureFormat.Alpha8, false);
                ...
                else
                {
                    if (m_bSupportRockchip)
                    {
                        m_VideoTexture = new Texture2D(Call_GetVideoWidth(), Call_GetVideoHeight(), TextureFormat.RGB565,
                            false);
                    }
                    else
                    {
                        m_VideoTexture = new Texture2D(Call_GetVideoWidth(), Call_GetVideoHeight(), TextureFormat.RGBA32,
                            false);
                    }
    ...
    public void Resize()
    {
        ...
        if (m_objResize != null)
        {
            ...
            for (int i = 0; i < m_objResize.Length; i++)
            {
                ...
                {
                    m_objResize[i].transform.localScale
                        = new Vector3(m_objResize[i].transform.localScale.x, m_objResize[i].transform.localScale.y,
                            m_objResize[i].transform.localScale.z);
                }
            }
        }
...
#endif
        //if (GetCurrentState() != MEDIAPLAYER_STATE.ERROR)
        //{
        Call_UnLoad();
        DeleteVideoTexture();
...
//    }
    ...
    {
        if (m_CurrentState == MEDIAPLAYER_STATE.PLAYING || m_CurrentState == MEDIAPLAYER_STATE.PAUSED ||
            m_CurrentState == MEDIAPLAYER_STATE.END || m_CurrentState == MEDIAPLAYER_STATE.READY ||
            m_CurrentState == MEDIAPLAYER_STATE.STOPPED)
        {
            m_fVolume = fVolume;
    ...
    {
        if (m_CurrentState == MEDIAPLAYER_STATE.PLAYING || m_CurrentState == MEDIAPLAYER_STATE.READY ||
            m_CurrentState == MEDIAPLAYER_STATE.PAUSED || m_CurrentState == MEDIAPLAYER_STATE.END ||
            m_CurrentState == MEDIAPLAYER_STATE.STOPPED)
    ...
    private unsafe bool Call_Load(string strFileName, int iSeek)
    {
        ...
        loader = new Thread(() =>
        {
            ...
            ffmpeg.av_dict_set(&opts, "stimeout", "20000000", 0);
            int i = ffmpeg.avformat_open_input(&ppFomatContext, strFileName, null, &opts);
            
            if ( i!= 0)
            {
                pFormatContext = null;
                m_CurrentState = MEDIAPLAYER_STATE.ERROR;
                Debug.Log("Could not open file"+" error("+i+") : " + strFileName);
            ...
            AddActionForUnityMainThread(() => { LoadVideoPart2(); });
        });
        ...
        if (m_CurrentState != MEDIAPLAYER_STATE.PAUSED)
        {
                        ...
                        if (m_bPC_FastMode == true)
                        {
                            ...
                            if (iLineSize != m_iWidth)
                            {
                                ...
                                for (int i = 0; i < m_iHeight/2; i++)
                                {
                                    ...
                                    imageBufferPtr = new IntPtr(convertedFrameAddress);
                                    Marshal.Copy(imageBufferPtr, buffer,
                                        m_iWidth*m_iHeight + m_iWidth/2*m_iHeight/2 + m_iWidth/2*i, m_iWidth/2);
                                }
                            }
                            else
                            {
            ...
            do
            {
                ...
                if (pStreamAudio != null)
                {
                    ...
                    {
                        int iAudioLen = ffmpeg.avcodec_decode_audio4(pAudioCodecContext, pDecodedAudioFrame, &gotSound,
                            pPacket);
                        if (iAudioLen >= 0)
                        {
                            if (gotSound == 1)
                            {
                                int iDataSize = ffmpeg.av_samples_get_buffer_size(null, pAudioCodecContext->channels,
                                    pDecodedAudioFrame->nb_samples, pAudioCodecContext->sample_fmt, 1);
                                int iDataSize2 = ffmpeg.av_samples_get_buffer_size(null, pAudioCodecContext->channels,
                                    pDecodedAudioFrame->nb_samples, AVSampleFormat.AV_SAMPLE_FMT_FLT, 1);
                                ...
                                //if( pAudioCodecContext->sample_fmt != AVSampleFormat.AV_SAMPLE_FMT_FLT)
                                {
                                    //for(int i = 0; i < pAudioCodecContext->channels; i++)
                                    {
                                        ...
                                        {
                                            long layout =
                                                ffmpeg.av_get_default_channel_layout(pAudioCodecContext->channels);
                                            pAudioCvtContext = ffmpeg.swr_alloc_set_opts(null, (long) layout,
                                                AVSampleFormat.AV_SAMPLE_FMT_FLT, pAudioCodecContext->sample_rate
                                                , (long) layout, pAudioCodecContext->sample_fmt,
                                                pAudioCodecContext->sample_rate, 0, (void*) 0);
                                        }
                                        else
                                        {
                                            pAudioCvtContext = ffmpeg.swr_alloc_set_opts(null,
                                                (long) pAudioCodecContext->channel_layout,
                                                AVSampleFormat.AV_SAMPLE_FMT_FLT, pAudioCodecContext->sample_rate
                                                , (long) pAudioCodecContext->channel_layout,
                                                pAudioCodecContext->sample_fmt, pAudioCodecContext->sample_rate, 0,
                                                (void*) 0);
                                        }
                                        ...
                                        ffmpeg.swr_convert(pAudioCvtContext, &outData, iDataSize2,
                                            pDecodedAudioFrame->extended_data, pDecodedAudioFrame->nb_samples);
                                        ...
                                        if (bSeekTo == true)
                                        {
                                            double value = pts*(double) pDecodedAudioFrame->sample_rate/
                                                           ((double) iDataSize2/4/pDecodedAudioFrame->channels);
                                        ...
                                        while (pts > 600.0f*(iInitCount + 1))
                                        {
                                            iSoundCount -=
                                                (int)
                                                    (600.0f*(double) pDecodedAudioFrame->sample_rate/
                                                     ((double) iDataSize2/4/pDecodedAudioFrame->channels));
                                            iInitCount++;
                                            ;
                                        }
                                        ...
                                        lock (listAudio)
                                        {
                                            ...
                                            lock (listAudioPts)
                                            {
                                                lock (listAudioPtsTime)
                                                {
                                                    ...
                                                    {
                                                        listAudioPts.Add(iSoundCount++*iDataSize2/4/
                                                                         pDecodedAudioFrame->channels);
                                                    }
    ...
    private void OnAudioRead(float[] data)
    {
        ...
        if (iSoundBufferCount < data.Length)
        {
            ...
            if ((data.Length - iSoundBufferCount) > listAudio[1].Length)
            {
                ...
                Array.Copy(listAudio[2], 0, data, iSoundBufferCount + listAudio[1].Length,
                    data.Length - iSoundBufferCount - listAudio[1].Length);
    ...
    private void Call_UpdateVideoTexture()
    {
        ...
        if (threadVideo == null && m_CurrentState != MEDIAPLAYER_STATE.END &&
            m_CurrentState != MEDIAPLAYER_STATE.NOT_READY && m_CurrentState != MEDIAPLAYER_STATE.ERROR)
        {
            threadVideo = new Thread(ThreadUpdate);
            threadVideo.IsBackground = true;
            threadVideo.Start();
        ...
        if (Mathf.Abs(fLastFrameTime - fCurrentSeekTime) < 0.1f)
        {
            for (int i = 0; i < listAudio.Count; i++)
            {
                if (listAudioPtsTime.Count > i)
                {
                    if (audioSource == null &&
                        (int)
                            ((float) pAudioCodecContext->sample_rate*
                             ((float) listAudioPtsTime[i] + ((float) Call_GetDuration()/1000.0f))) > 0)
                    {
	...
	#else
                        audioClip = AudioClip.Create("videoAudio",
                            (int) ((float) pAudioCodecContext->sample_rate*600.0f), pAudioCodecContext->channels,
                            pAudioCodecContext->sample_rate, false, false);
                        ...
                        if (listAudioPts[i] >= 0)
                        {

                            {
								...
								else*/
                                {
                                    ...
                                    audioClip.SetData(listAudio[i],
                                        (int) (listAudioPts[i]%(pAudioCodecContext->sample_rate*600.0f)));
                                }
                            }
                        }
                    }
            ...
            //do
            {
                ...
                if (listVideo.Count > 0)
                {
                    if (m_bPC_FastMode == true)
                    {
                        ...
                        SetTextureFromUnity(m_iID, (IntPtr) 0, m_texPtrY, m_texPtrU, m_texPtrV, m_iWidth, m_iHeight,
                            listVideo.Dequeue());
                    ...
                    else
                    {
                        ...
                        SetTextureFromUnity(m_iID, m_texPtr, (IntPtr) 0, (IntPtr) 0, (IntPtr) 0, m_iWidth, m_iHeight,
                            listVideo.Dequeue());
                        GL.IssuePluginEvent(GetRenderEventFunc(), 7000 + m_iID);
                    }
                }
	...
	audioSource.time = fLastFrameTime;
                    ...
                    for (int i = 0; i < m_TargetMaterial.Length; i++)
                    {
                        ...
                        if (m_bPC_FastMode == true)
                        {
                            if (m_TargetMaterial[i].GetComponent<MeshRenderer>() != null)
                            {
                                ...
                                if (m_TargetMaterial[i].GetComponent<MeshRenderer>().material.GetTexture("_Y") !=
                                    m_VideoTextureY)
                                {
                                    m_TargetMaterial[i].GetComponent<MeshRenderer>()
                                        .material.SetTexture("_Y", m_VideoTextureY);
                                }

                                if (m_TargetMaterial[i].GetComponent<MeshRenderer>().material.GetTexture("_U") !=
                                    m_VideoTextureU)
                                {
                                    m_TargetMaterial[i].GetComponent<MeshRenderer>()
                                        .material.SetTexture("_U", m_VideoTextureU);
                                }

                                if (m_TargetMaterial[i].GetComponent<MeshRenderer>().material.GetTexture("_V") !=
                                    m_VideoTextureV)
                                {
                                    m_TargetMaterial[i].GetComponent<MeshRenderer>()
                                        .material.SetTexture("_V", m_VideoTextureV);
                                }
                            }
                        }
                        ...
                        if (m_bPC_FastMode == true)
                        {
                            if (m_TargetMaterial[i].GetComponent<RawImage>() != null)
                            {
                                ...
                                if (m_TargetMaterial[i].GetComponent<RawImage>().material.GetTexture("_Y") !=
                                    m_VideoTextureY)
                                {
                                    m_TargetMaterial[i].GetComponent<RawImage>()
                                        .material.SetTexture("_Y", m_VideoTextureY);
                                }

                                if (m_TargetMaterial[i].GetComponent<RawImage>().material.GetTexture("_U") !=
                                    m_VideoTextureU)
                                {
                                    m_TargetMaterial[i].GetComponent<RawImage>()
                                        .material.SetTexture("_U", m_VideoTextureU);
                                ...
                                if (m_TargetMaterial[i].GetComponent<RawImage>().material.GetTexture("_V") !=
                                    m_VideoTextureV)
                                {
                                    m_TargetMaterial[i].GetComponent<RawImage>()
                                        .material.SetTexture("_V", m_VideoTextureV);
                ...
                if (bVideoFirstFrameReady == true)
                {
                    ...
                    {
                        if (audioSource == null
                            /*&& (int)((float)pAudioCodecContext->sample_rate * ((float)listAudioPtsTime[i] + ((float)Call_GetDuration() / 1000.0f))) > 0*/)
                        {
                            audioSource = gameObject.AddComponent<AudioSource>();
                        }
    ...
    private void Call_Play(int iSeek)
    {
        if (m_CurrentState == MEDIAPLAYER_STATE.READY || m_CurrentState == MEDIAPLAYER_STATE.STOPPED ||
            m_CurrentState == MEDIAPLAYER_STATE.END || m_CurrentState == MEDIAPLAYER_STATE.PAUSED)
        {
            SeekTo(iSeek);
            if (audioSource != null)
            {
